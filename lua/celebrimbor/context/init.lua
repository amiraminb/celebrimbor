  ...
  function M.get_type_definition(type_name)
    ...
    -- Find type_declaration that matches our type name
    for node in root:iter_children() do
      if node:type() == 'type_declaration' then
        local text = vim.treesitter.get_node_text(node, bufnr)
        -- Check if this type declaration contains our type name
  ...
  -- Get all function/method signatures in the file (excluding current one)
  ...
  -- Main function: gather all context for code generation
  function M.gather()
